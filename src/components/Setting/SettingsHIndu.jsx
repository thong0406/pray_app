...
const SettingsHIndu = () => {
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-50">
            <header className="bg-gradient-to-r from-[#5A1B0E] to-[#7A2D1E] text-center text-white py-3 text-xl font-semibold shadow-lg">
                Settings
            </header>
            
            <div className="flex-1 p-6 space-y-6">
                <div className="bg-white p-5 rounded-md shadow-lg transition-shadow duration-300">
                    <div 
                        className="flex justify-between items-center cursor-pointer" 
                        onClick={toggleAudioSection}
                    >
                        <h2 className="text-base font-semibold text-gray-800">Audio</h2>
                        <svg 
                            className={`w-6 h-6 text-gray-500 hover:text-gray-700 transition-transform duration-200 ${isAudioOpen ? 'rotate-180' : ''}`} 
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                        >
                    ...
                    <div className={`mt-4 transition-all duration-300 ease-in-out overflow-hidden ${isAudioOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                        <div className="mt-4">
                            <h3 className="text-sm font-medium text-gray-700">Music</h3>
                            <input 
                                type="range" 
                                min="0" 
                                max="100" 
                                className="w-full h-2 mt-2 bg-gray-300 rounded-lg appearance-none cursor-pointer hover:bg-gray-400 transition-all duration-200"
                                style={{ accentColor: '#5A1B0E' }} 
                            />
                        </div>
                        <div className="mt-4">
                ...
                <div className="bg-white p-5 rounded-md shadow-lg transition-shadow duration-300">
                    ...
                    >
                        ...
                        <svg 
                            className={`w-6 h-6 text-gray-500 hover:text-gray-700 transition-transform duration-200 ${isNotificationsOpen ? 'rotate-180' : ''}`} 
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                        >
                    ...
                    <div className={`mt-4 transition-all duration-300 ease-in-out overflow-hidden ${isNotificationsOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                        <div className="mt-4">
                            <label className="inline-flex items-center">
                                <input type="checkbox" className="form-checkbox text-[#5A1B0E] h-5 w-5" />
                ...
                <div className="bg-white p-5 rounded-md shadow-lg transition-shadow duration-300">
                    ...
                    >
                        <h2 className="text-base font-semibold text-gray-800">Language Selection</h2>
                        <svg 
                            className={`w-6 h-6 text-gray-500 hover:text-gray-700 transition-transform duration-200 ${isLanguageOpen ? 'rotate-180' : ''}`} 
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                        >
                    ...
                    <div className={`mt-4 transition-all duration-300 ease-in-out ${isLanguageOpen ? 'max-h-60 opacity-100 overflow-y-auto' : 'max-h-0 opacity-0'}`}>
                        ...
                        <div className="mt-4 space-y-2 max-h-40 overflow-y-auto">
                            <button className="w-full text-left p-3 bg-gray-100 rounded-md text-gray-700 hover:bg-gray-200 transition-colors duration-200">
                                Tiếng Việt
                            </button>
                            <button className="w-full text-left p-3 bg-gray-100 rounded-md text-gray-700 hover:bg-gray-200 transition-colors duration-200">
                                English
